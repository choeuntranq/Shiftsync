...
function EmployeesView({ employees, setEmployees, roles, availability }) {
  ...
  const invite = async () => {
    ...
    // Use Supabase's built-in invite — sends email with set-password link
    ...
    if (error) { setInvErr(error.message); setBusy(false); return }

    // Insert profile row immediately so they show up in the employee list
    ...
    setEmployees(emps||[])
    setShowInvite(false)
    setForm({name:'',email:'',phone:'',section:'FOH',roles:[],role_level:'employee'})
    setBusy(false)
  }
...
function ShiftTemplatesView({ roles, shiftBlocks, requirements, setRequirements, openDays }) {
  ...
  useEffect(()=>{
    sb.from('shift_templates').select('*').order('created_at').then(({data})=>setTemplates(data||[]))
  },[])

  const allRoles = [...(roles.FOH||[]),...(roles.BOH||[])]

  const saveTemplate = async () => {
    setSaving(true)
    const rec = {name:form.name, days:form.days, shift_block_id:form.shiftBlockId, role_counts:form.roleCounts}
    const {data} = await sb.from('shift_templates').insert(rec).select().single()
    if (data) setTemplates(p=>[...p,data])
    setShowNew(false)
    setForm({name:'',days:[],shiftBlockId:'',roleCounts:{}})
    setSaving(false)
  }
  ...
  const applyTemplate = async tmpl => {
    ...
    const blk = shiftBlocks.find(b=>b.id===tmpl.shift_block_id)
    if (!blk) return
    tmpl.days.forEach(day => {
      Object.entries(tmpl.role_counts||{}).forEach(([role,count]) => {
        if (!count) return
        if (!newReq[role]) newReq[role] = {}
        if (!newReq[role][day]) newReq[role][day] = {}
        newReq[role][day][tmpl.shift_block_id] = {count, startOverride:blk.start, endOverride:blk.end}
      })
    })
    setRequirements(newReq)
    await sb.from('shift_requirements').upsert({id:1, requirements:newReq})
    setApplied(tmpl.id)
    setTimeout(()=>setApplied(null), 2500)
  }

  const toggleDay = d => setForm(f=>({...f, days:f.days.includes(d)?f.days.filter(x=>x!==d):[...f.days,d]}))
  const setCount = (role, n) => setForm(f=>({...f, roleCounts:{...f.roleCounts, [role]:Math.max(0,n)}}))

  const TCSS = `.tcard{background:var(--s);border:1px solid var(--b);border-radius:12px;padding:18px;margin-bottom:12px;}
    .thead2{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px;}
    .daypill{padding:3px 10px;border-radius:100px;font-size:11px;font-weight:700;background:var(--s2);color:var(--t2);border:1px solid var(--b);}
    .daypill.on{background:rgba(255,92,53,.13);color:var(--a);border-color:rgba(255,92,53,.3);}
    .rcrow{display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--b);}
    .rcrow:last-child{border:none;}
    .cnt{display:flex;align-items:center;gap:8px;}
    .cnb{width:26px;height:26px;border-radius:6px;background:var(--s2);border:1px solid var(--b2);font-size:14px;font-weight:700;cursor:pointer;color:var(--t);display:flex;align-items:center;justify-content:center;}
    .cnb:hover{background:var(--s3);}
    .cnn{width:28px;text-align:center;font-weight:700;font-size:14px;}
  `
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:16,display:'flex',gap:10,alignItems:'center'}}>
        <p style={{fontSize:13,color:'var(--t2)',flex:1}}>Create named shift groups (e.g. "Lunch Days", "Dinner Only") and apply them to populate requirements on the schedule.</p>
        <button className="btn pr" onClick={()=>setShowNew(true)}><Ic p={P.plus} s={14}/> New Template</button>
      </div>
      ...
      {templates.length===0&&!showNew&&(
        <div className="card" style={{textAlign:'center',padding:40}}>
          ...
          <div style={{fontSize:13,color:'var(--t3)',marginBottom:16}}>Example: Create a "Lunch Days" template for Mon-Wed with Lunch shift counts, and a "Dinner Days" template for Thu-Sun with Dinner shift counts.</div>
          <button className="btn pr" onClick={()=>setShowNew(true)}><Ic p={P.plus} s={14}/> Create First Template</button>
        </div>
      )}
      ...
      {templates.map(tmpl => {
        const blk = shiftBlocks.find(b=>b.id===tmpl.shift_block_id)
        return (
          <div className="tcard" key={tmpl.id}>
            <div className="thead2">
              <div>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:16,marginBottom:8}}>{tmpl.name}</div>
                <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:10}}>
                  {DAYS.map(d=><span key={d} className={`daypill ${tmpl.days?.includes(d)?'on':''}`}>{d.slice(0,3)}</span>)}
                </div>
                {blk&&<span style={{fontSize:12,fontWeight:600,color:blk.color,background:`${blk.color}22`,padding:'2px 10px',borderRadius:100}}>{blk.name} · {blk.start}–{blk.end}</span>}
              </div>
              <div style={{display:'flex',gap:6,flexShrink:0}}>
                <button className={`btn ${applied===tmpl.id?'ok':'pr'} sm`} onClick={()=>applyTemplate(tmpl)}>
                  {applied===tmpl.id?'✓ Applied':'Apply to Schedule'}
                </button>
                <button className="btn gh ic" onClick={()=>deleteTemplate(tmpl.id)}><Ic p={P.trash} s={14} c="var(--re)"/></button>
              </div>
            </div>
            <div style={{background:'var(--s2)',borderRadius:8,padding:'8px 12px'}}>
              {Object.entries(tmpl.role_counts||{}).filter(([,c])=>c>0).map(([role,count])=>(
                <div key={role} className="rcrow">
                  <span style={{fontSize:13,fontWeight:500}}>{role}</span>
                  <span style={{fontSize:13,fontWeight:700,color:'var(--a)'}}>{count} staff</span>
                </div>
              ))}
              {Object.values(tmpl.role_counts||{}).every(c=>!c)&&<div style={{fontSize:12,color:'var(--t3)',textAlign:'center'}}>No role counts set</div>}
            </div>
          </div>
        )
      })}
      ...
      {showNew&&(
        <div className="ov" onClick={e=>e.target===e.currentTarget&&setShowNew(false)}>
          <div className="modal" style={{width:580}}>
            ...
            <div className="fg"><label className="lbl">Template Name</label><input className="fi" placeholder='e.g. "Lunch Days", "Weekend Dinner"' value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></div>
            <div className="fg">
              <label className="lbl">Days This Applies To</label>
              <div style={{display:'flex',gap:6,marginTop:6,flexWrap:'wrap'}}>
                {DAYS.map(d=><button key={d} onClick={()=>toggleDay(d)}
                  style={{padding:'5px 14px',borderRadius:100,cursor:'pointer',fontSize:12,fontWeight:700,border:'1px solid',background:form.days.includes(d)?'rgba(255,92,53,.15)':'var(--s2)',color:form.days.includes(d)?'var(--a)':'var(--t2)',borderColor:form.days.includes(d)?'rgba(255,92,53,.3)':'var(--b)'}}>
                  {d.slice(0,3)}
                </button>)}
              </div>
            </div>
            <div className="fg">
              <label className="lbl">Shift Block</label>
              <div style={{display:'flex',gap:8,marginTop:6,flexWrap:'wrap'}}>
                {shiftBlocks.map(b=><button key={b.id} onClick={()=>setForm({...form,shiftBlockId:b.id})}
                  style={{padding:'6px 16px',borderRadius:8,cursor:'pointer',fontSize:12,fontWeight:700,border:'1px solid',background:form.shiftBlockId===b.id?`${b.color}22`:'var(--s2)',color:form.shiftBlockId===b.id?b.color:'var(--t2)',borderColor:form.shiftBlockId===b.id?b.color:'var(--b)'}}>
                  {b.name} · {b.start}–{b.end}
                </button>)}
              </div>
            </div>
            <div className="fg">
              <label className="lbl">Staff Count Per Role</label>
              <div style={{background:'var(--s2)',borderRadius:8,padding:'8px 12px',marginTop:6}}>
                {['FOH','BOH'].map(sec=>(
                  <div key={sec} style={{marginBottom:12}}>
                    <div style={{fontSize:11,fontWeight:700,color:sec==='FOH'?'var(--a)':'var(--pu)',marginBottom:8,textTransform:'uppercase',letterSpacing:'.5px'}}>{sec}</div>
                    {(roles[sec]||[]).map(role=>(
                      <div key={role} className="rcrow">
                        <span style={{fontSize:13,fontWeight:500}}>{role}</span>
                        <div className="cnt">
                          <button className="cnb" onClick={()=>setCount(role,(form.roleCounts[role]||0)-1)}>−</button>
                          <span className="cnn">{form.roleCounts[role]||0}</span>
                          <button className="cnb" onClick={()=>setCount(role,(form.roleCounts[role]||0)+1)}>+</button>
                        </div>
                      </div>
                    ))}
            ...
            <div className="ma">
              ...
              <button className="btn pr" onClick={saveTemplate} disabled={!form.name||!form.days.length||!form.shiftBlockId||saving}>{saving?'Saving…':'Save Template'}</button>
            </div>
          </div>
        </div>
      )}
...
function ManagerDashboard() {
  ...
  const TITLES = {
    ...
    templates:   ['Shift Templates','Define shift groups by day and apply to requirements'],
        ...
        <div className="main">
          ...
          {view==='chat'
            ...
            :<div className="body">
               ...
               {view==='templates'   &&<ShiftTemplatesView roles={roles} shiftBlocks={shiftBlocks} requirements={requirements} setRequirements={setRequirements} openDays={openDays}/>}
